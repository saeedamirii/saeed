function initSequenceGame() {
    ...
    view.html(`
        ...
        <div class="game-container">
            <details class="game-guide">
                ...
                <p>دنباله اعداد را به خاطر بسپار و آن را تکرار کن. با هر اشتباه (پس از اتمام راهنماها) یک جان از دست می‌دهی.</p>
            </details>
            <header class="game-header">
                <div id="medals-container">
                    <span id="sq-medal-bronze" class="medal">🥉</span> <span id="sq-medal-silver" class="medal">🥈</span>
                    <span id="sq-medal-gold" class="medal">🥇</span> <span id="sq-medal-master" class="medal">💎</span>
                </div>
                <h1>دنباله سریع</h1>
                <div class="game-info">
                    <span>مرحله: <span id="sq-level-display">1</span></span>
                    <div id="sq-lives-display" class="lives"></div>
                    <span>بالاترین مرحله: <span id="sq-highscore-display">0</span></span>
                </div>
            </header>
            <main>
                ...
                <div class="input-wrapper">
                    <div id="sq-fake-input"></div>
                    <input type="text" id="sq-sequence-input" autocomplete="off" />
                </div>
                <div class="button-group" style="margin-top:1rem;">
                    <button id="sq-sequence-submit-btn" disabled>تایید</button>
                </div>
            </main>
    ...
    // Scoped DOM Elements
    const levelDisplay = view.find('#sq-level-display'), highscoreDisplay = view.find('#sq-highscore-display'),
          livesDisplay = view.find('#sq-lives-display'), displayText = view.find('#sq-display-text'),
          sequenceInput = view.find('#sq-sequence-input'), fakeInput = view.find('#sq-fake-input'),
          submitBtn = view.find('#sq-sequence-submit-btn'), startBtn = view.find('#sq-start-sequence-btn'),
          messageDisplay = view.find('#sq-message-display');
    ...
    function updateUI() {
        ...
        livesDisplay.html('');
        for (let i = 1; i <= 3; i++) {
            livesDisplay.append($('<span>').addClass('heart').toggleClass('lost', i > lives).text('♥'));
        }
    }
    ...
    async function displaySequence() {
        ...
        displayText.text('آماده؟');
        await sleep(1000);
        const displayTime = Math.max(300, 800 - (level * 30));
        for (const num of sequence) {
            displayText.text(num); await sleep(displayTime);
            displayText.text(''); await sleep(200);
        }
        displayText.text('؟');
        isPlaying = false; setInputsDisabled(false); sequenceInput.focus();
    }

    function handleCorrectAnswer() {
        level++;
        if (level > highScore) { highScore = level; localStorage.setItem('sequenceHighScore', highScore); }
    ...
    function startGame() {
        ...
        startBtn.text('...').prop('disabled', true);
        startRound();
    }
    ...
    return function cleanup() { clearInterval(timer); view.empty(); };
}
