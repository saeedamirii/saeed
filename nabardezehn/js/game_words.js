function initWordRecallGame() {
    ...
    const highscoreDisplay = view.find('#wr-highscore-display'), levelDisplay = view.find('#wr-level-display'),
          livesDisplay = view.find('#wr-lives-display'), timerDisplay = view.find('#wr-timer-display'),
          themeDisplay = view.find('#wr-theme-display'), wordList = view.find('#wr-word-list'),
    ...
    const wordBank = { "میوه‌ها": ["سیب", "موز", "گیلاس"], "حیوانات": ["شیر", "فیل", "ببر"] };
    ...
    function startRound() {
        ...
        themeDisplay.text(`موضوع: ${currentTheme}`);
    ...
    function checkResults() {
        ...
        if (allCorrect) {
            extraLives++; level++;
            if (level > highScore) { highScore = level; localStorage.setItem('wordGameHighScore', highScore); }
            showWrView('levelComplete');
            setTimeout(() => startRound(), 1500);
        } else {
            if (extraLives > 0) { extraLives--; startRound(); } 
            else { finalLevelDisplay.text(level); showWrView('gameOver'); }
        }
    }

    function initializeGame() {
        level = 1; extraLives = 0; highScore = localStorage.getItem('wordGameHighScore') || 1;
